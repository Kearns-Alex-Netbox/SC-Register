...
Public Class AddBoard
	...
	Private Sub SaveButton_Click() Handles SaveButton.Click
		...
		While startingnumber <= endingnumber
			...
			If sqlapi.FindBoardSerialNumber(myCmd, myReader, currentBoardSNO, "") = True Then
				fullResult = fullResult & currentBoardSNO & " already added." & vbNewLine
				startingnumber += 1
				Continue While
			End If

			If sqlapi.AddBoard(myCmd, myConn, currentBoardSNO, record, result, False, CurrentRevision_TextBox.Text) = False Then
			...
			If BoardComment.Text.Length <> 0 Then
				If sqlapi.AddBoardComment(myCmd, currentBoardSNO, batchString & BoardComment.Text, record, result) = False Then
					fullResult = fullResult & "ERROR: " & result & vbNewLine
					startingnumber += 1
				End If
			End If
	...
	Private Sub BoardSerialNumber_LostFocus(sender As Object, e As EventArgs) Handles BoardSerialNumber.LostFocus
		Dim startingSNO As String = BoardSerialNumber.Text
		...
		If lastboard.Equals(board) = False Then

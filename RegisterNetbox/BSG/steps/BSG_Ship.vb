...
Public Class BSG_Ship
	...
	Private Sub UpdateButton_Click() Handles UpdateButton.Click
		...
		For Each row As DataRow In shipTable.Rows
			...
			If sqlapi.BSG_FindSystemSerialNumber(myCmd, row("Serial Number"), results) Then

				sqlapi.BSG_GetSystemDate(row("Serial Number"), "RegisterDate", registerDateTime, results)
				sqlapi.BSG_GetSystemDate(row("Serial Number"), "ParameterDate", parameterDateTime, results)
				sqlapi.BSG_GetSystemDate(row("Serial Number"), "BurnInDate", burnInDateTime, results)
				sqlapi.BSG_GetSystemDate(row("Serial Number"), "CheckoutDate", checkoutDateTime, results)

				Dim systemType As String = ""
				sqlapi.BSG_GetSystemCurrentType(row("Serial Number"), systemType, results)

				'First check to see if we did not get any dates.
				If registerDateTime = Nothing Then
					addString &= "MAC Address,  "
				End If
			...
			If row("Serial Number") <> Nothing Then
				If row("Serial Number").Length <> 0 Then
					...
					If updateAll = True Then
						resultBoolean = sqlapi.BSG_ShipAndInvoice(row("Serial Number"), CB_ShipOption.SelectedValue, skippedStepsSNOs, result, infoDate)
					Else If skippedStepsSNOs.Contains(row("Serial Number")) = false then
						resultBoolean = sqlapi.BSG_ShipAndInvoice(row("Serial Number"), CB_ShipOption.SelectedValue, blankList, result, infoDate)
					End If
			...
			If runningTransaction <> True And SerialNo.Text.Length <> 0 And UpdateButton.Focused <> True Then
				...
				If Queue_ListBox.FindString(SerialNo.Text) = -1 Then
					If sqlapi.BSG_FindSystemSerialNumber(myCmd, SerialNo.Text, result) = False Then
						...
						If sqlapi.BSG_FindBoardSerialNumber(myCmd, SerialNo.Text, result) = False Then
	...
	Private Sub PreShip_Button_Click() Handles PreShip_Button.Click
		...
		If Print_CheckBox.Checked = True Then
			PrintLabel(Ship_NUD.Value, shipTable)
		End If
